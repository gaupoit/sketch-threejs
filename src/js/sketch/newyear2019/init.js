...
export default async function () {
  ...
  const resizeCamera = () => {
    camera.setFocalLength(
      Math.min(resolution.x / 1200, 1) *
        50 *
        MathEx.step(1, resolution.x / resolution.y) +
        Math.min(resolution.x / 818, 1) *
          50 *
          MathEx.step(1, resolution.y / resolution.x)
    );
  ...
  const on = () => {
    ...
    if (os === "iOS" || os === "Android") {
      window.addEventListener("deviceorientation", (event) => {
        ...
        const y =
          MathEx.radians(event.gamma) * Math.cos(MathEx.radians(event.beta));
        boarHead.rotate(x, y);
      });
    } else {
  ...
  const obj = await objLoader.loadAsync(
    "/sketch-threejs/model/newyear2019/boar_head.obj"
  );

...
export default class Sparks extends THREE.InstancedMesh {
  constructor() {
    ...
    const ibaDirections = new THREE.InstancedBufferAttribute(
      new Float32Array(COUNT * 3),
      3
    );
    const ibaTimes = new THREE.InstancedBufferAttribute(
      new Float32Array(COUNT),
      1
    );
    const ibaDurations = new THREE.InstancedBufferAttribute(
      new Float32Array(COUNT),
      1
    );
    const ibaDistances = new THREE.InstancedBufferAttribute(
      new Float32Array(COUNT),
      1
    );
    const ibaScales = new THREE.InstancedBufferAttribute(
      new Float32Array(COUNT),
      1
    );
    ...
    const ibaUvDiffs = new THREE.InstancedBufferAttribute(
  ...
  update(time, core) {
    const { iPosition, iDirection, iTime, iDuration } =
